...
class CSVFile:
    ...
    def __init__(self, name):
        ...
        self.can_read = True #riesco a leggere il file
    
        try:
            my_file = open(self.name, 'r')
            my_file.readline()
        except Exception as e: #salvo l'eccezione nella variabile e
            self.can_read = False
    ...
    def get_data(self):
        ...
        if not self.can_read: #se non riesco a leggerlo
...
# Se nell'init ho settato can_read a False vuol dire che il file non poteva essere aperto o era illeggibile
        ...
        # Leggo il file linea per linea
            ...
            # p.s. in realta' strip() toglie anche gli spazi bianchi all'inizio e alla fine di una stringa.
...
class NumericalCSVFile(CSVFile): #estendendo la classe non sto riscrivendo l'init, mentre sto sovrascrivendo la get_data
    
    def get_data(self):
        
    # Chiamo la get_data del genitore (variabile che ha il contenuto della funzione iniziale)
    ...
    # Preparo lista per contenere i dati ma in formato numerico
        ...
        for string_row in string_data: #andava bene anche for element in string_data
            
        # Preparo una lista di supporto per salvare la riga in "formato" numerico (tranne il primo elemento)
            numerical_row = []
            
        # Ciclo su tutti gli elementi della riga con un enumeratore: cosi' ho gratis l'indice "i" della posizione dell'elemento nella riga.
            ...
            # Converto a float tutto gli altri. Ma se fallisco, stampo l'errore e rompo il ciclo (e poi saltero' la riga).
                    try:
                        numerical_row.append(float(element)) #dal secondo elemento
                    except Exception as e:
                        print('Errore in conversione del valore "{}" a numerico: "{}"'.format(element, e))
                        break #se ho un errore esco dal for e guardo se sono riuscita a processare tutti gli elementi 
