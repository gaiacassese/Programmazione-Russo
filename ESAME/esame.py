...
class CSVTimeSeriesFile:
    ...
    def get_data(self):
        ...
        # la userò per vedere se è ordinata o se ci sono duplicati
        listaMesiAnni = []
        ...
        try:
            ...
            raise ExamException('File non trovato o non leggibile! errore: "{}" '.format(e))
        ...
        #non serve che controlli se ci sono più di due elementi perchè la funzione split prende i primi due in automatico
                ...
                #controlla che la data sia valida, se non lo è salto la riga senza alzare eccezioni
                try:
                    testData = data.split("-")
                    datetime.datetime(int(testData[0]),int(testData[1]),1)
                except:
                    print("impossibile convertire la data --> {} - SKIP".format(data))
                    continue
                    
                #controlla che il numero di passeggeri sia valido, se non lo è salto la riga senza alzare eccezioni
                try:
                    passeggeri = int(elements[1])
                except:
                    print("numero di passeggeri non numerico --> {} - SKIP".format(elements[1]))
                    continue
...
#CONTROLLO I PARAMETRI
    ...
    if time_series is None:
        print("lista time_series None")
        raise ExamException("La lista time_series è None")
        
    if len(time_series)==0:
        print("lista time_series ha lunghezza 0")
        raise ExamException("La lista time_series ha lunghezza 0")
        
    #controllo years
    #se la lista è None
    if years is None:
        print("lista anni None")
        raise ExamException("La lista degli anni è None")
    
    #che la lista abbia 2 valori
    if len(years) != 2:
        print("gli anni devono essere 2")
        raise ExamException("gli anni devono essere 2")
    
    # valori interi per gli anni
    if type(years[0]) is not int:
        raise ExamException("il primo anno non è intero")
        
    if type(years[1]) is not int:
        raise ExamException("il secondo anno non è intero")
        
    # anni ordinati consecutivi
    if years[1] != years[0] + 1:
        raise ExamException("il secondo anno deve essere successivo al primo")
    ...
    for anno in years:
        ...
        if not found:
            raise ExamException('Anno non nella lista "{}"'.format(anno))
    ...
    #ci saranno 11 segnaposti in quanto la lista delle differenze avrà un posto in meno rispetto alla lista in cui vado a prendere i dati
